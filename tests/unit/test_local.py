...
Simple test runner for local development/Docker environments
"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"LOCAL API TESTS - {BASE_URL}")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        print(f"  Make sure the API is running locally or in Docker\n")
        sys.exit(1)
    ...
    report_file = report_dir / "local_test_results.json"
    ...
    simple_report = report_dir / "local_test_summary.txt"
    with open(simple_report, "w") as f:
        f.write(f"LOCAL TEST RESULTS - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"{'='*60}\n\n")
